        ...
        success: function (response) {
            if (response.status === 'partial') {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const successfulItems = Object.values(selectedData).filter(item => {
                            const key = `${item.product_id}_${item.option_id}`;
                            return !failed_keys.includes(key);
                        });
                        ...
                        if (successfulItems.length > 0) {
                            ...
                            Swal.fire({ icon: 'info', title: 'No valid products to add.' });
                        }
                    }
                });
                ...
                Swal.fire({ icon: 'error', title: response.message || 'Error' });
            }
        },
        error: function () {
            Swal.fire({
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status === 'partial') {
                ...
                response.errors.forEach(function (err) {
                    errorList += `<li><span style="font-family:poppins;font-size:14px;">${err.name}</span> — ${err.message}</li>`;
                    failed_keys.push(`${err.product_id}_${err.option_id}`);
                });
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const successfulItems = all_fav.filter(item => {
                            const key = `${item.product_id}_${item.option_id}`;
                            return !failed_keys.includes(key);
                        });
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'partial') {
                        ...
                        Swal.fire({ icon: 'error', title: response.message || 'Validation error' });
                    }
                },
                error: function () {
                    Swal.fire({
