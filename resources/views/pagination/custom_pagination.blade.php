    ...
    <ul class="pager m-0 p-0">
        <div class="row main-row">
            <div class="col-md-3 pagination previous_btn">
                ...
                @else
                    <li>
                        <a class="btn btn-flat prev_btn_css" href="{{ $paginator->previousPageUrl() }}" rel="prev"> <i
                                class="fa fa-arrow-left mr-1" style="color:#4B4B4B; font-size:13px;"></i>
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <li class="active my-active pd_nmber"><span class="">{{ $page }}</span>
                                </li>
                            @else
                ...
                @if ($paginator->hasMorePages())
                    <li>
                        <a class="btn btn-flat next_btn_css" href="{{ $paginator->nextPageUrl() }}" rel="next">
                            <span class="prev_next_btn mr-1">Next</span>
