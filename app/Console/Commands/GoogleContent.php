...
class GoogleContent extends Command
{
    ...
    private function insertProducts($client, $token)
    {
        ...
        $all_products_ids = Product::whereIn('category_id', $product_categories)->pluck('product_id')->toArray();
        ...
        $product_pricing_option_ids = Pricingnew::whereIn('option_id', $product_options_ids)
            ...
            ->pluck('option_id')->toArray();
        $products_ids = ProductOption::whereIn('option_id', $product_pricing_option_ids)->pluck('product_id')->toArray();
        ...
        if ($products->isNotEmpty()) {
            foreach ($products as $product) {
                foreach ($product->options as $option) {
                    ...
                    if ($product->categories) {
                        ...
                        } elseif ($product->categories->parent_id != 0) {
                    ...
                    $product_array[] = [
                        ...
                        'title' => $product->name,
                        'code' => $product->code,
                        'description' => $product->description ? strip_tags($product->description) : 'No description available',
                        'link' => url('product-detail/' . $product->id . '/' . $option->option_id . '/' . $product->slug),
                        'image_link' => $product->product_image->image ? url(asset('theme/products/images/' . $product->product_image->image)) : url(asset('theme/img/image_not_available.png')),
                        'price' => $option->price[0]->$price_column ?? 0,
                        'condition' => 'new',
                        'availability' => $option->stockAvailable > 0 ? 'In stock' : 'Out of stock',
            ...
            foreach ($product_array as $add_product) {
