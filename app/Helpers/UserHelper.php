...
class UserHelper
{
    ...
    public static function  get_stock_per_product_option($id, $option_id) {
        ...
        if (!empty($product_option) && intval($product_option->stockAvailable) > 0) {
            $stock = $product_option->stockAvailable;
        } else {
            $stock = 0;
        }
        return $stock;
    ...
    public static function shipping_order($order_id , $currentOrder , $order_contact) {
        ...
        $data = [
            ...
            'confirmation' => floatval($currentOrder->productTotal) > 499 ? 'signature' : 'delivery',
            'items'=> $items
        ];
        $headers = [
